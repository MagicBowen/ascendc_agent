...
SHORT=r:,v:,i:,b:,p:,d:,
LONG=run-mode:,soc-version:,install-path:,build-type:,install-prefix:,data-type:,
...
DATA_TYPE_LIST="float16 float32"
if [[ " $DATA_TYPE_LIST " != *" $DATA_TYPE "* ]]; then
    echo "[ERROR]: DATA_TYPE should be in [$DATA_TYPE_LIST]"
    exit -1
fi
...
echo "[INFO]: Current data type is ${DATA_TYPE}"
...
python3 scripts/gen_data.py ${DATA_TYPE}
(
    ...
    if [[ "$RUN_WITH_TOOLCHAIN" -eq 1 ]]; then
        if [ "${RUN_MODE}" = "npu" ]; then
            ...
            ./ascendc_kernels_bbit ${DATA_TYPE}
        fi
    else
        ./ascendc_kernels_bbit ${DATA_TYPE}
    fi
...
md5sum output/*.bin
python3 scripts/verify_result.py output/output_z.bin output/golden.bin ${DATA_TYPE}